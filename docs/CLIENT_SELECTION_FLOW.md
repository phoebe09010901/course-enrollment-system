...
本文件定義「三款樣板提案與限時選版流程」。之後為新課程產生三款樣板時，不是每次重新發明三款，而是必須從 `docs/TEMPLATE_REFERENCE.md` 的 10 套樣板資料中挑選合適組合。
...
1. 客戶資料完成建檔後，專案狀態變成「待樣板提案」。
2. Chat A 根據課程類型，從 `docs/TEMPLATE_REFERENCE.md` 的 10 套樣板資料中挑選 3 款方向。
3. Chat A 使用 Canva 製作 3 款視覺樣板。
4. 每款樣板先做核心區塊即可：
   - Hero 主視覺
   - About / 課程介紹
   - Gallery / 作品展示
   - CTA / 報名區
5. 這 3 款樣板是方向選擇，不是三份完整網站。
6. 客戶從 3 款中選 1 款。
7. 選定後才進入完整前端實作、報價、專案流程。
8. 客戶不可任意混搭三款，避免失控。
9. 可提供一次小幅調整，例如色系、圖片順序、CTA 文案、局部版面微調。
10. 不允許無限修改。
...
Canva 樣板連結或截圖: <Canva URL or screenshot path>
是否入選: false
有效期限: YYYY-MM-DD
```
...
- 不可讓客戶無限制混搭三款方向。
- 不可把三款方向都做成完整網站後再讓客戶選，三款只做核心視覺區塊。
