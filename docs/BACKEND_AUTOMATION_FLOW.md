...
本文件規劃「課程招生 - 系統」從網頁表單送出、資料庫建檔、後台查看、三款樣板預覽、通知客戶，到三天後預覽網址作廢的完整後端與自動化流程。
...
建議建立此表保存 LINE AI 接待、流程說明與系統問題對話，方便追蹤客戶從 LINE 進入表單的來源。

LINE AI 不再負責整理表單資料、補問缺漏或輸出建檔 JSON；表單送出資料應由表單後端或 `form_submissions` / `course_intake_submissions` 類資料表保存。
...
| `intent` | text | 否 | LINE AI 判定的接待意圖，例如 `start_form`、`ask_process`、`tech_support` |
| `form_url_sent` | text | 否 | 若曾提供表單網址，記錄當下使用的 URL |
| `support_category` | text | 否 | 網站 / 系統問題分類 |
| `created_at` | timestamp | 是 | 建立時間 |
| `notes` | text | 否 | 備註 |

### `form_submissions`

建議建立此表保存網頁表單送出的原始資料與驗證狀態，作為 `course_intake_form_submitted` worker 的輸入來源。

| 欄位 | 建議型別 | 必填 | 說明 |
| --- | --- | --- | --- |
| `submission_id` | uuid / text pk | 是 | 表單送出主鍵 |
| `source` | text | 是 | 例如 `public_course_intake_form` |
| `line_user_id` | text | 否 | 若由 LINE 導流可保存來源 |
| `client_id` | uuid / text fk | 否 | 建檔後回填 |
| `project_id` | uuid / text fk | 否 | 建檔後回填 |
| `raw_payload` | json | 是 | 表單原始送出資料 |
| `validation_status` | text | 是 | `valid`、`needs_data`、`invalid` |
| `missing_fields` | json | 否 | 缺少欄位 |
| `worker_run_id` | text | 否 | 處理此 submission 的 worker run |
| `submitted_at` | timestamp | 是 | 表單送出時間 |
| `processed_at` | timestamp | 否 | worker 處理完成時間 |
| `created_at` | timestamp | 是 | 建立時間 |
| `updated_at` | timestamp | 是 | 更新時間 |

## 表單送出 Payload

網頁表單送出後，後端應交給 worker 類似以下 payload：

```json
{
  "submission_id": "sub_20260528_001",
...
- `submission_id`
- `client.client_name`
- `client.email` 或 `client.phone` 至少一個
- `course_project.course_name`
- `course_project.course_type`
- `course_project.needs_template_proposal`
- `submitted_at`

`line_user_id` 可作為來源追蹤與 LINE 通知輔助，但不應再作為建檔唯一入口；表單可用 Email / phone / line_user_id 做去重策略。
...
- 建立 `course_projects` 後，若表單必填驗證通過且需要提案，狀態直接設為 `待樣板提案`。
...
- `course_intake_form_submitted`：接收網頁表單送出 payload，建立或更新客戶與課程專案。
...
- 網頁表單送出。
- 後端收到 `course_intake_form_submitted` payload。

流程：

1. 接收表單送出 payload。
2. 將原始 payload 寫入 `form_submissions` 或等效 submission log。
3. 檢查必填欄位是否完整。
4. 若缺少欄位，將 submission 標記為 `needs_data`，並在後台顯示缺漏欄位；不回到 LINE AI 補問。
5. 若完整，依 `line_user_id`、email 或 phone 查找既有 `clients`。
6. 建立或更新 `clients`。
7. 建立 `course_projects`。
8. `project_status` 設為 `待樣板提案`。
9. 寫入 `notification_logs`，類型可為 `data_confirmed`。
10. 通知後台有新案件。

建議 worker task 名稱：

- `course_intake_form_submitted`
