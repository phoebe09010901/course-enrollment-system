...
- 已同步更新 automation prompt 的 infrastructure preflight：
  ...
  - 若 run context 指向 `4d27`、`1210`、舊的 `945c` 或任何非 `e89a` cwd，必須立刻停止。
...
本次驗證沒有呼叫 claim endpoint，沒有 claimed project。

## 剩餘風險

- 若下一輪 automation 仍從 `4d27`、`1210`、`945c` 或其他非 `e89a` cwd 啟動，代表問題不在 repo 或 `automation.toml` 內容，而是 scheduler / run session cache 層仍有舊上下文。
