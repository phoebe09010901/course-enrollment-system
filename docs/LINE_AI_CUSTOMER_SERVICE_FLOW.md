...
21. 聯絡欄位更新期間必須鎖定單一欄位並保存原流程位置；更新完成後回到原本缺的課程欄位、照片階段或摘要確認，不可重新跳到照片第 1 階段。
22. 更新 Email 時若格式錯誤，需明確提示 Email 格式錯誤；不可清掉姓名或 LINE ID Link。
23. 更新 LINE ID Link 時若客戶誤貼 Email，不可把 Email 當 LINE Link，也不可完成 LINE Link 更新。
...
- 課程類型說明：指課程的主題分類，例如水彩、色鉛、色鉛筆、營養宣導、手作、花藝、美甲、攝影、設計、瑜伽、烘焙。
...
- contact 階段單字型課程形式 / 類型答案不可推測為姓名，例如 `實體`、`線上`、`混合`、`畫畫`、`色鉛`、`色鉛筆`、`水彩`、`營養宣導`。
...
- contact update resume guard：開始更新聯絡欄位時保存 `resume_intake_step`；更新成功後依原步驟恢復，若原本在摘要確認則重建摘要並等待「確認」。
- course name parser guard：只有 `課程名稱：...` 時不可從課名自動推測 `course_type`，避免課程類型被課名污染。
...
6k. contact update matrix：更新 Email / 姓名 / LINE ID Link 後必須回到原流程；連續更新三個欄位後，確認 payload 必須使用最新值。
