...
網站工廠使用 `docs/WEBSITE_FACTORY_TEMPLATE_REGISTRY.md` 作為目前的樣板登記表。

目前登記：

- `style-001` 第一個風格：`template-001`、`template-002`、`template-003`。
- `style-002` 第二個風格：`template-004`、`template-005`、`template-006`，目前狀態為 `draft`。

任何 chat 不得只靠資料夾名稱判斷 template 歸屬。Chat M-D / M-B / M-A / M-C 都必須依 template registry 判斷 `style_family_id`、`template_id` 與 `template_status`。
...
- 樣板登記表，例如 `docs/WEBSITE_FACTORY_TEMPLATE_REGISTRY.md`，清楚定義 template 與 style family 的關係。
