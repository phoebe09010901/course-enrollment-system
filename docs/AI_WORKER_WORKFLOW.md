...
- Chat G automation 在 claim 前必須先執行 `scripts/chat-g-network-preflight.sh`，固定記錄 `pwd`、`runtime_context`、DNS resolver、Python `socket.gethostbyname_ex("ftm.com.tw")`、system DNS probe、public DNS probe、health curl timing 與 health response。
- `scripts/chat-g-network-preflight.sh` 的順序必須是：先驗證 cwd，再做 `socket.gethostbyname_ex("ftm.com.tw")` DNS probe，再做 health endpoint probe。
- `scripts/chat-g-network-preflight.sh` 保留 3 次 bounded DNS retry，每次間隔 2 到 5 秒；若三次 socket DNS 都失敗，必須寫入 `dns_resolution_failed` 並停止，不可進入 health、claim、success/fail callback 或 Canva generation。
- 若 manual shell 的 socket DNS 成功，但 automation runtime 的 socket DNS 失敗，優先查 scheduler/runtime network context，不要先查 backend/API/Canva。
- 若 automation archived session 顯示 `Network access is restricted`，這是 runtime capability blocker；不可用增加 retry 次數、改回 `worktree` mode 或硬編 IP 作為主要修復。
