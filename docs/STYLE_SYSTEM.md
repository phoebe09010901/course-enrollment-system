...
- 所有版型不要出現英文裝飾標題。
- 禁止 `WORKSHOP`、`COURSE`、`GALLERY`、`ABOUT`、`OFFER`、`REGISTER` 等英文背景大字。
- 所有主標、副標、區塊標題以中文為主。
- 桌機版字級不要過大。
- 不追求巨大標題；走精緻、穩重、留白感。
- Hero 可以有份量，但不可造成壓迫感。
- 不要額外新增「聯絡我們」獨立區塊。
- 聯絡資訊應整合在課程資訊、報名 CTA 或 Footer。
- 不要資訊塞滿。
- 高級感來自分層、留白、圖片比例與節奏。
- 圖片要大，留白要夠。
- 動畫不是第一優先，版型美感才是第一優先。
- 動畫以輕微 fade-up、image reveal、慢速淡入為主。
- 使用 Google Fonts 時，需遵守統一 Typography 規範。
- 主要內文使用 `Noto Sans TC`；主要標題使用 `Noto Serif TC`；柔和輔助可使用 `Zen Maru Gothic`。
