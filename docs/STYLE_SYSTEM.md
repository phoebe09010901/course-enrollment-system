...
這些文件仍是 foundation，不代表已有實際網站、實際品牌案例或可直接套用的完整模板。
...
| course-brand-template-v1 | 已有 `styles/course-brand-template-v1.json` foundation，尚未建立 `templates/` 實體模板 |
...
目前已有 `styles/course-brand-template-v1.json` 作為設計 foundation，但尚未建立 `templates/course-brand-template-v1/` 的實體模板。
...
- course-brand-template-v1 的實體模板與範例頁面。
